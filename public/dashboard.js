...
fetch('/api/auth/session', { credentials: 'include' })
    ...
    .then(data => {
        ...
        if (data.user) {
            crmUserRole = data.user.role || '';
            crmUserPermissions = Array.isArray(data.user.permissions) ? data.user.permissions : [];
        }
        loadDashboard();
        ...
        const normalizedPage = pageParam === 'crm' ? 'leads' : pageParam;
        if (normalizedPage && document.querySelector(`#dashboardSidebar [data-page="${normalizedPage}"]`)) {
            showPage(normalizedPage);
        }
    })
    .catch(err => {
...
// Leads — apenas Kanban (leads-kanban.js)
async function loadLeads() {
    if (typeof loadCRMKanban === 'function') await loadCRMKanban();
    else if (typeof loadKanbanBoard === 'function') await loadKanbanBoard();
}
...
function updateCrmUserAvatarPreview(name, avatarUrl) {
    ...
    if (crmUserAvatarPendingFile) {
        /* preview handled by FileReader on change */
    } else if (!crmUserAvatarRemove && avatarUrl) {
                ...
                tbody.innerHTML = data.data
                    .map((u) => {
                        ...
                        const mustPw = u.must_change_password
                            ? ' <span class="badge badge-warning" title="Trocar senha">senha</span>'
                            : '';
                        const actions = [];
                        if (canEdit)
                            actions.push(
...
function updateUsersPageActions() {
    const btn = document.getElementById('crmNewUserBtn');
    if (!btn) return;
    const canCreate = crmUserRole === 'admin' || crmUserPermissions.includes('users.create');
...
async function fetchPermissionRegistry() {
    ...
    const j = await res.json();
    if (j.success) permissionRegistryCache = j;
    return j;
}
...
window.openCrmUserModal = openCrmUserModal;
window.closeCrmUserModal = closeCrmUserModal;
window.showNewUserModal = showNewUserModal;
window.deactivateCrmUser = deactivateCrmUser;
